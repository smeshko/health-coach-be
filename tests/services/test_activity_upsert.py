...
def test_same_date_twice_in_one_batch_does_not_error(session) -> None:
    # The intra-batch dedupe (last-wins) guards against SQLite's "cannot UPSERT a
    # row twice" when one payload carries the same date more than once.
    days = upsert_activity(
        session,
        [
            _summary("2026-06-01", activeEnergyKcal=620.0),
            _summary("2026-06-01", activeEnergyKcal=700.0),
        ],
    )
    assert days == 1  # distinct dates
    assert _count(session) == 1  # one row
    row = session.execute(
        select(ActivitySummaryRow).where(ActivitySummaryRow.date == "2026-06-01")
    ).scalar_one()
    assert row.active_energy_burned == 700.0  # last wins
